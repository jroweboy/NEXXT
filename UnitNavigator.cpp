...
void __fastcall TFormNavigator::Draw(bool doSnap, bool doAlign)
{
	...
	/*
	bProcessDrawNavOn=true;
	TBulkDrawNav *ProcessDrawNav = new TBulkDrawNav(false);
	*/
        			...
        			for(j=0;j<minW;j++)
        			{
        				if(j>=0&&j<w&&i>=0&&i<h)
        				{
        					...
        					else
        					{
        						...
        						{
        							FormMain->DrawTile(Map1->Picture,x,y,tile,FormMain->AttrGet(j,i,false,false),j,i,false,false,1,true,bHalfsize);
        						}
        						else
        						{
			...
			for(j=0;j<tw;j++)
			{
				...
				if(j>=0 && tx+j<w && i>=0 && ty+i<h) 
				{
					...
					else
					{
						if(!FormMain->SpeedButtonSelTiles->Down||chrSelected[tile])
						{
							FormMain->DrawTile(Map1->Picture,x,y,tile,FormMain->AttrGet(tx+j,ty+i,false,false),tx+j,ty+i,false,false,1,true,bHalfsize);
						}
						else
						{
